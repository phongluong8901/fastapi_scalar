...
# Lớp cha cho tất cả các loại lỗi của dự án, kế thừa từ Exception của Python
...
# Ví dụ một lớp con kế thừa từ FastShipError: EntityNotFound
...
def _get_handler(status: int, detail: str):
    # Hàm này tạo ra một "hàm con" (handler) để thực hiện xử lý cụ thể
    def handler(request: Request, exception: Exception) -> Response:
        # DEBUG PRINT STATEMENT 👇
        from rich import print, panel   # In lỗi ra màn hình console (dùng thư viện Rich để hiển thị đẹp mắt)
        print(
            panel.Panel(
...
def add_exception_handlers(app: FastAPI):
    ...
    exception_classes = FastShipError.__subclasses__()  # Lấy danh sách tất cả các class con của FastShipError (tự động phát hiện lỗi mới)

    for exception_class in exception_classes:
        # Add exception handler
        app.add_exception_handler(  # Đăng ký lỗi với ứng dụng FastAPI
    ...
    # Đăng ký trình xử lý riêng cho lỗi hệ thống (HTTP 500)
    @app.exception_handler(status.HTTP_500_INTERNAL_SERVER_ERROR)
    def internal_server_error_handler(request, exception):
        return JSONResponse(
            content={"detail": "Something went wrong..."},  # Trả về thông báo chung chung
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            headers={
                "X-Error": f"{exception}",  # Đẩy lỗi chi tiết vào header để debug
            }
        )
